#!/usr/bin/bash -l
#SBATCH -p short -c 64 --mem 96gb --out logs/make_pairs_h2_hap01.log 
...
file=$SEARCH/Trebouxia_sp._hap02_hap01.FASTA.tab.gz
perl scripts/make_pair_seqfiles.pl $file
prefix=$(basename $file .FASTA.tab.gz)
echo $prefix
parallel -j $CPUS muscle -threads 2 -align {} -output $TEMP/{/}.afa ::: $(ls ${PAIRS}/$prefix/*.pep)
